...
# ============================================================================
# IndexValuation — 指数/行业市值表（全市场成交的输入，不是个股加总）
# ============================================================================

SCHEMA_INDEX_VALUATION = Schema(
    name="IndexValuation",
    index_names=["date"],
    columns=[
        ColumnSpec("symbol", "str", nullable=False, description="指数代码（如 000985.CSI）"),
        ColumnSpec("turnover", "float64", nullable=False, description="换手率（小数，jq 百分数 / 100）"),
        ColumnSpec("circulating_mcap", "float64", nullable=False, description="流通市值（元）"),
        ColumnSpec("amount", "float64", nullable=False, description="成交额 = 换手 × 流通市值（元）"),
        ColumnSpec("market_cap", "float64", description="总市值（元）"),
    ],
    invariants=[
        "turnover ≥ 0；circulating_mcap > 0；amount ≈ turnover × circulating_mcap",
        "同一 date 唯一；单位已换成 qlab（元 / 小数），不是聚宽的 亿元 / 百分数",
    ],
)
