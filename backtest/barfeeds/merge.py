...
from .util import _validate_datetime_index, _generate_trading_time_index
...
def bar_merge(df: pd.DataFrame, trading_periods: List[Tuple[str, str]], from_freq: str, to_freq: str) -> pd.DataFrame:
    ...
    for expected_index_time in expected_index:
        loc_list = [expected_index_time]
        for i in range(1, freq_times):
            loc_list.insert(0, expected_index_time - from_freq_td * i)
        processed_df.add(_bar_merge(df.loc[loc_list]))
