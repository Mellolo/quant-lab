...
class AbstractStrategy(bt.Strategy):
    ...
    def open_market(self, data, size, is_buy=True, target_price = None, stop_price = None):
        ...
        if target_price:
            self.position[order.ref]["mark_take_profit"] = target_price

        if stop_price:
            self.position[order.ref]["mark_stop_loss"] = stop_price
    ...
    def take_profit(self, order_ref, target_price):
        # 获取开仓订单
        order = self.position.get(order_ref, {}).get("open_order", None)
        if not order:
        ...
        # 如果已进入平仓状态，不允许下止盈单
        close_order = self.position.get(order_ref, {}).get("close_order", None)
    ...
    def cancel_take_profit(self, order_ref):
        if not self.position.get(order_ref, {}).get("open_order", None):
            raise ValueError(f"取消止盈单但未找到仓位，开仓订单编号: {order_ref}")

        # 不再需要notify处理
        self.position[order_ref]["mark_take_profit"] = None

        # 取消之前设置的止盈单
        take_profit_order = self.position.get(order_ref, {}).get("take_profit_order", None)
    ...
    def stop_loss(self, order_ref, stop_price):
        ...
        if not is_cancel:
            ...
            self.position[order_ref]["mark_stop_loss"] = stop_price
    ...
    def cancel_stop_loss(self, order_ref):
        ...
        # 不再需要notify处理
        self.position[order_ref]["mark_stop_loss"] = None

        # 取消之前设置的止损单
        stop_loss_order = self.position.get(order_ref, {}).get("stop_loss_order", None)
        if stop_loss_order:
    ...
    def close_position(self, order_ref):
        ...
        if not take_profit_cancel and not stop_loss_cancel:
            ...
            self.position[order_ref]["close_order"] = close_order
        else:
    ...
    def _notify_order_type(self, order):
        ...
        for order_ref in self.position:
            ...
            # 止盈订单
            take_profit_order = self.position[order_ref].get("take_profit_order", None)
            if take_profit_order and take_profit_order.ref == order.ref:
                return "take_profit", order_ref
            # 止损订单
            stop_loss_order = self.position[order_ref].get("stop_loss_order", None)
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            ...
            if order_type == "open":
                ...
                if not self.t1:
                    # 设置止盈
                    take_profit_price = self.position.get(position_ref, {}).get("mark_take_profit", None)
                    if take_profit_price is not None:
                        self.take_profit(position_ref, take_profit_price)
                    # 设置止损
                    stop_loss_price = self.position.get(position_ref, {}).get("mark_stop_loss", None)
                    ...
                    take_profit_price = self.position.get(position_ref, {}).get("mark_take_profit", None)
                    if take_profit_price is not None:
                        self.position[position_ref]["mark_take_profit"] = None
                        self.position[position_ref]["mark_take_profit_t1"] = take_profit_price
                    # 设置止损
                    stop_loss_price = self.position.get(position_ref, {}).get("mark_stop_loss", None)
                    if stop_loss_price is not None:
                        self.position[position_ref]["mark_stop_loss"] = None
                        self.position[position_ref]["mark_stop_loss_t1"] = stop_loss_price

            elif order_type in ["close", "take_profit", "stop_loss"]:
                self.cancel_take_profit(position_ref)
                self.cancel_stop_loss(position_ref)
                del self.position[position_ref]
            ...
            if order_type == "take_profit":
                self.position[position_ref]["take_profit_order"] = None
                # 若有，设置新止盈
                take_profit_price = self.position.get(position_ref, {}).get("mark_take_profit", None)
                if take_profit_price is not None:
                    self.take_profit(position_ref, take_profit_price)
            ...
            if order_type == "stop_loss":
                self.position[position_ref]["stop_loss_order"] = None
                # 若有，设置新止损
                stop_loss_price = self.position.get(position_ref, {}).get("mark_stop_loss", None)
                if stop_loss_price is not None:
                    self.stop_loss(position_ref, stop_loss_price)
            ...
            if order_type in ["take_profit", "stop_loss"]:
                ...
                take_profit_order = self.position.get(position_ref, {}).get("take_profit_order", None)
                stop_loss_order = self.position.get(position_ref, {}).get("stop_loss_order", None)
                if not take_profit_order and not stop_loss_order:
                    if self.position.get(position_ref, {}).get("mark_close", False):
